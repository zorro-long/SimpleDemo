...
- (void)viewDidLoad {
  ...
  self.btnLapPercent.btnName = @"Lap%";
  self.btnNewPrice.btnName = @"Price";
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
  ...
  if (cell == nil) {
    NSArray *nib = [[NSBundle mainBundle] loadNibNamed:@"ListCell" owner:nil options:nil];
...
- (void)sortDatasource:(NSString *)key andIsAscending:(BOOL)isAscending {
  ...
  if ([key isEqualToString:SORT_KEY_CURRENT_PRICE]) {
    sortDescriptor = [[NSSortDescriptor alloc] initWithKey:@"currentPrice" ascending:isAscending];
    
  } else if ([key isEqualToString:SORT_KEY_LAP]) {
    sortDescriptor = [[NSSortDescriptor alloc] initWithKey:@"lap" ascending:isAscending];
    
  } else if ([key isEqualToString:SORT_KEY_LAP_PERCENT]) {
    sortDescriptor = [[NSSortDescriptor alloc] initWithKey:@"lapPercent" ascending:isAscending];
  }
  
  NSArray *sortDescriptors = [NSArray arrayWithObject:sortDescriptor];
  self.arrDatasource = [[NSMutableArray alloc] initWithArray:[self.arrDatasource sortedArrayUsingDescriptors:sortDescriptors]];
}

#pragma mark - Get Data

- (void)initDatasource {
  if (self.arrDatasource != nil && self.arrDatasource.count > 0) {
    [(NSMutableArray *)self.arrDatasource removeAllObjects];
  }
